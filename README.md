...
**v0.8.20** — 91 C# commands (86 exposed as MCP tools + 5 hidden) + 1 batch tool + 2 workflow recipes = **89 MCP tools**. Hidden = `create_spot_elevation` + the 4-command spatial-QC HTTP pack (`spatial_*`), registered for HTTP `/mcp` use but off the MCP tool surface.
...
# → ok=True, service=revit-mcp-addin, version=0.8.20, authEnabled=True
```
